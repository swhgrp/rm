...
async def get_transfers(
    ...
    """Get transfers with optional filtering (filtered by user's assigned locations)"""
    ...
    # Apply user location filtering - users can only see transfers involving their locations
    ...
    if user_location_ids is not None:  # None means admin (access all)
        if len(user_location_ids) == 0:
            ...
            query = query.filter(Transfer.id == -1)  # Return empty
        else:
            # User can see transfers from or to their locations
            query = query.filter(
                or_(
                    Transfer.from_location_id.in_(user_location_ids),
                    Transfer.to_location_id.in_(user_location_ids)
                )
            )
    ...
    # If user is staff, only show transfers they requested
